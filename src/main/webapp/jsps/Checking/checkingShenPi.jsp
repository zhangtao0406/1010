    ...
    <div class="modal fade" id="myModal2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" >
        <div class="modal-dialog" style="width: 900px; margin-top: 50px">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" onclick="checkingBuTongGuo()" class="btn btn-danger">审批不通过</button>
                    <button type="button" onclick="checkingTongGuo()" class="btn btn-primary">审批通过返回申请人</button>
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                </div>
            </div>
        </div>
...
<!-- ----------------------------以上考勤统计管理审批的模态框-------------------------分割线----------------------------------结束------------------ ------->
<!-- ----------------------------以上考勤统计管理审批选择审批人的模态框------------------------分割线----------------------------------开始------------------ ------->
<div>
...
<script type="text/javascript">
    ...
    function CheckingLike() {
        $.ajax({
            ...
            url: "${pageContext.request.contextPath}/CheckingController/findCheckingShenPiAll",
            dataType: "json",
                    ...
                    formatter: function (value, row, index) {
                        if (row.checkingStatus == 1) {
                            return "<a class='btn btn-info' onclick='CheckingAllByIdMyModal(" + row.id + ")' role='button' >查看</a>&nbsp;"+
                                "<a class='btn btn-success' onclick='CheckingTongJiMyModal(" + row.id + ")' role='button' >审批</a>";
    ...
    function checkingBuTongGuo() {
        ...
        if (window.confirm("是否对本次审批不通过?")) {
            $.ajax({
                type: "post",
                url: "${pageContext.request.contextPath}/CheckingController/checkingBuTongGuo",
    ...
    function  checkingTongGuo() {
        ...
        if (window.confirm("是否确认通过审批?")) {
            $.ajax({
                type: "post",
                url: "${pageContext.request.contextPath}/CheckingController/checkingTongGuo",
                data: {id:id,approvalName:approvalName,comment:comment}, //要传过去的数据$('#addInfrastructure').serialize()
                dataType: "json",
                success: function (data) {
                    if (data > 0) {
